/**
 * Rosna multi-tier SaaS plans (matrix product): Free + Pro + Pro+ + Ultra.
 * Not used for Kisti / other single-Pro products.
 *
 * Usage:
 *   APITO_GRAPHQL_ENDPOINT=... APITO_API_KEY=... \
 *   ROSNA_PROJECT_ID=rosna_v2_jpn6o deno run -A scripts/upsert-rosna-plans.ts
 */
...
async function main() {
  ...
  const roOpts = {
    projectId: process.env.ROSNA_PROJECT_ID || process.env.APITO_PROJECT_ID || "rosna_v2_jpn6o",
  };
  ...
  const plans = [
    {
      ...
      name: "Free",
      description:
        "Free: food_order cap 100; no CSV/Excel; no ledger/reports UI; employee writes blocked",
      logic_executions: ["*"],
      quotas: { "max_records.food_order": 100 },
    ...
    },
    {
      ...
      name: "Pro",
      description:
        "Pro: food_order cap 500; CSV import/export only (soft UI); no Excel; no ledger/reports UI; employee unlocked",
      logic_executions: ["*"],
      quotas: { "max_records.food_order": 500 },
      api_permissions: { "*": all },
      prices: price(299),
      provider_products: playSku("rosna_pro_monthly"),
      currency: "BDT",
      price_monthly: 299,
    },
    {
      ...
      name: "Pro Plus",
      description:
        "Pro+: food_order cap 1000; CSV+Excel; ledger/reports; full features",
      logic_executions: ["*"],
      quotas: { "max_records.food_order": 1000 },
      api_permissions: { "*": all },
      prices: price(799),
      provider_products: playSku("rosna_pro_plus_monthly"),
      currency: "BDT",
      price_monthly: 799,
    },
    {
      id: "ultra",
      name: "Ultra",
      description: "Ultra: unlimited food_order quota; all features",
      logic_executions: ["*"],
      quotas: {},
      api_permissions: { "*": all },
      prices: price(1499),
      provider_products: playSku("rosna_ultra_monthly"),
      currency: "BDT",
      price_monthly: 1499,
    },
  ];
