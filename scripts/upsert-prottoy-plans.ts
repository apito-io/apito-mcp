/**
 * Prottoy multi-tier SaaS plans (matrix product): Free + Pro + Pro+ + Ultra.
 * Same price ladder as Suchok; quota is lab_order; Play SKUs are prottoy_*.
 *
 * Usage:
 *   APITO_GRAPHQL_ENDPOINT=... APITO_API_KEY=... \
 *   PROTTOY_PROJECT_ID=prottoy_cjiot deno run -A scripts/upsert-prottoy-plans.ts
 */
...
async function main() {
  ...
  const roOpts = {
    projectId:
      process.env.PROTTOY_PROJECT_ID ||
  ...
  const plans = [
    {
      ...
      name: "Free",
      description:
        "Free: lab_order cap 100; no CSV/Excel; no ledger/reports UI; employee writes blocked",
      logic_executions: ["*"],
      quotas: { "max_records.lab_order": 100 },
    ...
    },
    {
      ...
      name: "Pro",
      description:
        "Pro: lab_order cap 500; CSV import/export only (soft UI); no Excel; no ledger/reports UI; employee unlocked",
      logic_executions: ["*"],
      quotas: { "max_records.lab_order": 500 },
      api_permissions: { "*": all },
      prices: price(299),
      provider_products: playSku("prottoy_pro_monthly"),
      currency: "BDT",
      price_monthly: 299,
    },
    {
      ...
      name: "Pro Plus",
      description:
        "Pro+: lab_order cap 1000; CSV+Excel; ledger/reports; full features",
      logic_executions: ["*"],
      quotas: { "max_records.lab_order": 1000 },
      api_permissions: { "*": all },
      prices: price(799),
      provider_products: playSku("prottoy_pro_plus_monthly"),
      currency: "BDT",
      price_monthly: 799,
    },
    {
      id: "ultra",
      name: "Ultra",
      description: "Ultra: unlimited lab_order quota; all features",
      logic_executions: ["*"],
      quotas: {},
      api_permissions: { "*": all },
      prices: price(1499),
      provider_products: playSku("prottoy_ultra_monthly"),
      currency: "BDT",
      price_monthly: 1499,
    },
  ];
